...
const Scroll = (props: { children: React.ReactNode, gap: string, className?: string }): React.ReactNode => {
  ...
  useLayoutEffect(() => {
    ...
    if (scroller != null) {
      ...
      const allBlocks: NodeListOf<HTMLElement> = scroller?.querySelectorAll(`.${styles.blocks}`)
  ...
  return <div onScroll={onScroll} className={`${styles.scroller} ${props.className ?? ''}`} ref={scrollerRef as React.LegacyRef<HTMLDivElement>}>
    {props.children}
  </div>
}
...
const Item = (props: { children: React.ReactNode, className?: string }): React.ReactNode => {
  return (
